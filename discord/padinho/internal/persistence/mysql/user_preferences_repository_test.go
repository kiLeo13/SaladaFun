...
func TestUserPreferencesRepositoryTogglesOuroquestAtomically(t *testing.T) {
	database, mock := preferenceMockDatabase(t)
	repository := NewUserPreferencesRepository(database)
	...
	mock.ExpectQuery("SELECT .*users_preferences").WithArgs(uint64(1), 1).WillReturnRows(sqlmock.NewRows([]string{"user_id", "auto_mudae_oq", "created_at", "updated_at"}).AddRow(1, false, 10, 20))
	enabled, err := repository.ToggleAutoMudaeOQ(1, true)
	if err != nil || enabled {
		t.Fatalf("ToggleAutoMudaeOQ() = %t, %v", enabled, err)
	}
	assertPreferenceExpectations(t, mock)
}
